...
class AnnouncementHelper {
    ...
    suspend fun getAnnouncements(teamId: String): List<AnnouncementModel>? {
        return try {
            val data = mapOf("teamId" to teamId)
            val result = functions.getHttpsCallable("getAnnouncements").call(data).await()
            ...
            Log.w(TAG, "Failed to get announcements: ${e.message}", e)
            null
        } catch (e: Exception) {
    ...
    suspend fun createAnnouncement(announcement: AnnouncementModel): Boolean {
        return try {
            ...
            Log.w(TAG, "Failed to create announcement: ${e.message}", e)
            false
        } catch (e: Exception) {
            Log.w(TAG, "Unknown error occurred: ${e.message}", e)
            false
